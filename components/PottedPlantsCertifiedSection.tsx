...
export default function PottedPlantsCertifiedSection() {
  return (
    <div className="relative w-full overflow-hidden">
      {/* Dekorasi daun — 1 gambar utuh, absolute, nembus batas section 1 (hijau)
          ke section 2 (putih) di pojok kanan-bawah. z-20 supaya tampil DI ATAS
          kedua background section (yang solid/opaque). Posisi & sisi dibalik
          (kanan, mirror) dibanding punya GrowHigherSection biar variatif. */}
      <div className="pointer-events-none absolute -right-6 top-[260px] z-20 w-[220px] -scale-x-100 opacity-70 sm:top-[320px] sm:w-[300px] lg:w-[360px]">
        <Image
          src={LEAF_DECOR}
          alt=""
          width={480}
      ...
      <section className="relative z-10 grid h-[624px] w-full bg-[#FAFFF0] lg:grid-cols-2">
        ...
        <div
          ...
          style={{ paddingRight: "max(24px, calc((100vw - 1400px) / 2 + 24px))" }}
        >
          <h2 className="text-[28px] font-bold leading-tight sm:text-[40px]" style={{ color: DARK_GREEN }}>
            The Perfect Choice for <br/>Potted Plants
          </h2>
          <p className="mt-1 text-[19px] font-bold leading-snug sm:text-[30px]" style={{ color: LIGHT_GREEN }}>
            Grow Better in Every Pot.
          </p>
          <p className="mt-4 max-w-[560px] text-[15px] leading-relaxed text-[#606060] sm:text-base">
            mGRO is a lightweight aggregate that enhances potted plants and landscaping applications by
            reducing the weight of growing media while improving drainage and aeration. Suitable for mixing
            with soil or soilless media (e.g. coco coir, coco peat, peat moss), it also functions as a
            drainage layer or decorative top dressing. mGRO helps create a less favourable environment for
            certain pests, supporting cleaner and healthier planting spaces.
          </p>
        </div>
      </section>
      ...
      <section className="relative z-10 w-full bg-white px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-5xl text-center">
          <h2 className="inline-flex items-center gap-2 text-[28px] font-bold leading-tight sm:text-[40px]">
            <span>
              <span style={{ color: DARK_GREEN }}>Certified Green</span>
              <br />
              <span style={{ color: DARK_GREEN }}>For a Greener Future</span>
            </span>
            <Image
              src={LEAF_ICON_SMALL}
              alt=""
            ...
            {certifications.map((cert) => (
              ...
              >
                <Image
                  ...
                  className="h-30 w-30"
                />
                <h3 className="mt-4 text-[17px] font-bold leading-snug" style={{ color: LIGHT_GREEN }}>
                  {cert.titleLines[0]}
                  <br />
                  {cert.titleLines[1]}
                </h3>
